...
export class CS571HW10TokenAgent {

    public static readonly DEFAULT_EXP: number = 60 * 60;

    private readonly config: CS571Config<HW10PublicConfig, HW10SecretConfig>;

    public constructor(config: CS571Config<HW10PublicConfig, HW10SecretConfig>) {
        this.config = config;
    }
    ...
    public generateAccessToken = (user: BadgerUser, exp?: number): string => {
        return this.generateToken({ ...user }, exp ?? CS571HW10TokenAgent.DEFAULT_EXP);
    }
